...
After the user approves the draft, **deepen each FR one at a time** through focused Q&A. This is where FRs gain the precision needed for AI-agent implementation.
...
Probe for: algorithms, boundary conditions, data flow specifics, error handling, configuration vs hardcoded decisions. When available, examine real data/config files to inform questions.

**Note any Section 8 material discovered during deepening** (algorithms, error codes, format specifications) — do not write Section 8 yet; step 5 handles it.
